...
void sendMeasurements() {
  ...
  // SANITIZATION: Запобігаємо відправці NaN або критичних значень, що ламають схему API
  if (isnan(currTemp) || currTemp < -50.0 || currTemp > 100.0) {
    Serial.println("[WARN] Invalid Temp, using default 22.0");
    currTemp = 22.0;
  }
  if (isnan(currHum) || currHum < 0.0 || currHum > 100.0) {
    Serial.println("[WARN] Invalid Humidity, using default 50.0");
    currHum = 50.0;
  }
  if (currSoil < 0) currSoil = 0;
...
void pollCommands() {
  if (WiFi.status() != WL_CONNECTED || isPumpRunning) return; // Не запитуємо команди, якщо помпа вже працює

  unsigned long timestamp = getTimestamp();
  if (timestamp == 0) return;
  
  // ... (решта коду pollCommands до моменту обробки PUMP_WATER) ...
      ...
      for (JsonObject cmd : commands) {
        ...
        if (cmdType == "PUMP_WATER") {
          ...
          if (water_ml > 0) {
            ...
            delay(300); // Чекаємо стабілізації після WiFi
        ...
        else if (cmdType == "PUMP_WATER_SEC") {
          float duration_sec = cmdPayload["duration_sec"] | 0.0;
          if (duration_sec > 0) {
            unsigned long duration_ms = (unsigned long)(duration_sec * 1000);
            delay(300); // Чекаємо стабілізації після WiFi
            pinMode(PUMP_PIN, OUTPUT); 
            digitalWrite(PUMP_PIN, LOW); 
            pumpTurnOffTime = millis() + duration_ms;
...
void setup() {
  ...
  Serial.println("\n[SETUP] MoonPlants V2 FIX 400 ERROR");
  ...
  if (!bmp.begin(0x76)) {
    Serial.println("[STEP 4] BMP280 FAILED");
...
void loop() {
  ...
  if (currentMillis - lastDisplayRefresh >= 2000) {
    ...
    if (!isnan(t)) currTemp = t;
    if (!isnan(h)) currHum = h;
