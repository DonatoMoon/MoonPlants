...
export async function waterPlantSecAction(input: z.infer<typeof schema>) {
    const user = await getCurrentUser();
    if (!user) throw new AppError('unauthorized', 'Не авторизовано', 401);

    const rl = rateLimit(`water:${user.id}`, 20, 60_000);
    if (!rl.ok) throw new AppError('rate_limited', 'Забагато запитів, спробуйте за хвилину', 429);

    const parsed = schema.parse(input);
    const result = await IoTService.waterPlantSec(user.id, parsed.plantId, parsed.durationSec);
