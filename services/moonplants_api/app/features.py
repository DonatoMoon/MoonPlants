...
def add_moisture_lag_rolling(
    ...
    for pid, grp in df.groupby("plant_instance_id", sort=False):
        ...
        if "vpd_kpa" in grp.columns:
            ...
            # 1. Integral stress (VPD Area) over last 6 hours (36 steps)
            grp["vpd_integral_6h"] = vpd_shifted.rolling(window=36, min_periods=1).sum()

        # 2. Interaction between light and moisture deficit
        light_shifted = grp["light_lux"].shift(1)
        grp["light_moisture_stress"] = light_shifted / (sm_shifted + 0.01)

        # 3. Second derivative of moisture (Acceleration of drying)
        grp["moisture_speed"] = sm_shifted.diff(periods=6)
        grp["moisture_acceleration"] = grp["moisture_speed"].diff(periods=6)
...
def get_base_feature_cols(df: pd.DataFrame) -> list[str]:
    ...
    exclude = {
        ...
        "plant_instance_id", "species_id", # Excluded to prevent ID memorization/leakage
